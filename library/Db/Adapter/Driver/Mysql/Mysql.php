...
use Yau\Db\Adapter\Exception\RuntimeException;
use Yau\Db\Sql\Sql;

/**
* Database adapter for mysql resource connections
*
* @author   John Yau
* @category Yau
* @package  Yau_DB
...
* @throws RuntimeException if using a non-SELECT query
*/
public function getOne($query, array $params = array())
{
	...
	if ($result === TRUE)
	{
		throw new RuntimeException('Cannot fetch from a non-SELECT query');
	}
...
{
	$result = \mysql_query('BEGIN', $this->dbh);
